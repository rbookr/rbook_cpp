...
struct BTreeNode
{
     ...
     */
    template<typename U,
        std::enable_if_t<std::is_same_v<std::decay_t<U>,T>, bool> = true>
    void insertNonFull(U&& k);
...
template<typename T,size_t deg>
    template<typename U,
        std::enable_if_t<std::is_same_v<std::decay_t<U>,T>, bool> >
void BTreeNode<T,deg>::insertNonFull(U&& k){
    int i  = bsearch(k); //i是第一个>k的位置
    if( leaf == true){ //这个叶子
        for(int j = n-1;j >=i;j--) //后移
            keys[j+1] = std::move(keys[j]);
        keys[i] = std::forward<U>(k); 
        n++;
    }
    else { // 这个是 notFull
        //i--; //i是后一个<=k的位置
        ...
        left(i)->insertNonFull(std::forward<U>(k));
    }
}
...
template<typename U,
    std::enable_if_t<std::is_same_v<std::decay_t<U>,T>, bool>>
void BTree<T,deg>::insert(U&& k){ //遍历
    //空的结点
    if( root == nullptr){
        root = node::getNewNode();
        root->keys[0] = std::forward<U>(k);
        root->n = 1;
    }
    else { //Btree 不空
        if( root->isFull() ) { 
            ...
            //现在root有两个孩子,决定使用哪个
            if(root->keys[0] > k)
                root->left(0)->insertNonFull(std::forward<U>(k));
            else
                root->right(0)->insertNonFull(std::forward<U>(k));
        }
        else
            root->insertNonFull(std::forward<U>(k));
    }
}
